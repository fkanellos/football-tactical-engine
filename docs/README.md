...
| [ball-tracking-design.md](ball-tracking-design.md) | The **other missing input**: the diagnosis that the deployed pipeline contains **no ball detector at all** (person-only filter, read from TrackLab source — upstream GSR excludes the ball by design), the verified literature (SoccerNet precedents, small-ball detectors, motion models, ball-from-players inference), the no-ground-truth ball measurement methodology, the per-frame `ball_quality` gate, a ball-appropriate motion model, and the ranked plan starting with a measure-first GPU probe. | Measurement layer + motion model **implemented + tested** (synthetic trajectories, 42 tests, `pipeline/ball/`); probe script **committed** (`research/ball_probe.py`); first real ball evidence & Phase 4 wiring **pending next GPU session**. |
